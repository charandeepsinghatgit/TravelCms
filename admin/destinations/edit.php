...
=======
if ($res->num_rows === 0) { echo "Not found"; require_once __DIR__ . '/../includes/admin_footer.php'; exit; }
>>>>>>> 4b0b099 (Initial commit: Travel CMS project)
$row = $res->fetch_assoc();
...
=======
    ...
    if ($name === '' || $country === '') {
        $errors[] = 'Name and country required.';
    }
    if ($youtube_id !== '' && !is_valid_youtube_id($youtube_id)) {
        $errors[] = 'Invalid YouTube ID';
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("
            UPDATE destinations
            SET name = ?, country = ?, description = ?, rating = ?, date_added = ?, image = ?, youtube_id = ?
            WHERE id = ?
        ");
        $stmt->bind_param('sssdsssi', $name, $country, $desc, $rating, $date_added, $image_to_store, $youtube_id, $id);
  ...
  <form method="post" enctype="multipart/form-data">
    <div class="mb-3">
      ...
      <input name="name" class="form-control"
             value="<?php echo e($_POST['name'] ?? $row['name']); ?>">
    </div>

    <div class="mb-3">
      <label class="form-label">Country</label>
      <input name="country" class="form-control"
             value="<?php echo e($_POST['country'] ?? $row['country']); ?>">
    </div>

    <div class="mb-3">
      <label class="form-label">Description</label>
      <textarea name="description" class="form-control"><?php
        echo e($_POST['description'] ?? $row['description']);
      ?></textarea>
    </div>

    <div class="mb-3">
      <label class="form-label">Rating</label>
      <input name="rating" type="number" step="0.1" class="form-control"
             value="<?php echo e($_POST['rating'] ?? $row['rating']); ?>">
    </div>

    <div class="mb-3">
      <label class="form-label">Date Added</label>
      <input name="date_added" type="date" class="form-control"
             value="<?php echo e($_POST['date_added'] ?? $row['date_added']); ?>">
    </div>
    ...
    <div class="mb-3">
      ...
      <?php if ($row['image']): ?>
        <div class="mb-2">
          <!-- ✅ from /admin/destinations/ go up 2 levels to /uploads/destinations/ -->
          <img src="../../uploads/destinations/<?php echo e($row['image']); ?>" style="max-width:200px">
        </div>
      <?php endif; ?>
      <input type="file" name="image" accept="image/*" class="form-control">
    </div>
    ...
    <div class="mb-3">
      ...
      <input name="youtube_id" class="form-control"
             value="<?php echo e($_POST['youtube_id'] ?? $row['youtube_id']); ?>">
    </div>

    <button class="btn btn-primary">Save</button>
    <!-- ✅ back to same folder's index -->
